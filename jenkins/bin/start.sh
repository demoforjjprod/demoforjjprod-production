...
REPO=$LOGNAME
IMAGE_NAME="demoforjjprod-jenkins"
REPO=demoforjjprod
...
CONTAINER_IMG="$(sudo docker ps -a -f name=demoforjjprod-jenkins-dood --format "{{ .Image }}")"
...
JENKINS_MOUNT="-v demoforjjprod-jenkins-home:/var/jenkins_home -e DOCKER_JENKINS_MOUNT=demoforjjprod-jenkins-home:/var/jenkins_home"
...
then
    ...
    then
        ...
        echo "#!/bin/sh
sleep 30
docker rm -f demoforjjprod-jenkins-dood
sleep 2

docker run --restart always $DOCKER_DOOD -d -p $SERVICE_PORT:8080 $JENKINS_MOUNT --name demoforjjprod-jenkins-dood $GITHUB_USER $ADMIN $CREDS $PROXY $DOCKER_OPTS $TAG_NAME
...
sudo -E docker run --restart always $DOCKER_DOOD -d -p $SERVICE_PORT:8080 $JENKINS_MOUNT --name demoforjjprod-jenkins-dood $GITHUB_USER $ADMIN $CREDS $PROXY $DOCKER_OPTS $TAG_NAME

if [ $? -ne 0 ]
then
    echo "Issue about jenkins startup."
    sudo docker logs demoforjjprod-jenkins-dood
    exit 1
fi
